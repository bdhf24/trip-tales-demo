...
-- 3. Kid Interests: Add interests array to kids table
ALTER TABLE kids 
ADD COLUMN IF NOT EXISTS interests text[] DEFAULT ARRAY[]::text[];

-- Add index for better query performance on kid interests
CREATE INDEX IF NOT EXISTS idx_kids_interests ON kids USING GIN(interests);

-- Add index for pages with high-res vs preview
CREATE INDEX IF NOT EXISTS idx_pages_is_high_res ON pages(is_high_res);

-- Update the update_updated_at trigger for kids table to capture interest changes
